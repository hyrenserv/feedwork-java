 ...
 *                 .append(field1, rhs.getField1())
 *                 .append(field2, rhs.getField2())
 *                 .isEquals();
 *  }
 * </pre>
 *
 ...
 * }
 * </pre>
 * 这种用法，因为内部使用的是反射，所以性能稍差
 *
 * <p>{@link ExcludedField} 注解可用于排除字段.</p>
 */
...
//				final String fieldName = f.getName();
				...
				if (!ArrayUtil.contains(excludeFields, fieldName)
						...
						&& !f.isAnnotationPresent(ExcludedField.class)) {
	...
	public EqualsBuilder append(final Object lhs, final Object rhs) {
		...
		if (lhsClass.isArray()) {
			...
			if (testRecursive && !ClassUtil.isPrimitiveOrWrapper(lhsClass)) {
				reflectionAppend(lhs, rhs);
			} else {
				isEquals = lhs.equals(rhs);
			}
