 ...
 *
 * <p>
 * 可以通过调用 appendSuper ，把父类的hashCode加进来
 * </p>
 *
 * 使用方式 2：
 * <pre>
 ...
 * }
 * </pre>
 * 由于使用了反射，比方式1要慢，而且，如果有 security manager ，会导致失败。
 *
 * 对于 reflectionHashCode 方式，可使用 {@link HashCodeExclude} 注解，排除掉不需要的 field
 */
...
//				String fieldName = field.getName();
			...
			// 用缓存的方式，替换上面4句，运行效率有一定提升：1万次循环，110毫秒->40毫秒 的提升
			Map<String, Field> fieldMap = BeanUtil.getDeclaredFields(clazz);
			Set<Map.Entry<String, Field>> fieldsSet = fieldMap.entrySet();
			for (final Map.Entry<String, Field> m : fieldsSet) {
				final String fieldName = m.getKey();
				final Field field = m.getValue();

				if (!ArrayUtil.contains(excludeFields, fieldName)
						&& !fieldName.contains("$")
						&& (useTransients || !Modifier.isTransient(field.getModifiers()))
						&& !Modifier.isStatic(field.getModifiers())
						&& !field.isAnnotationPresent(ExcludedField.class)) {
	...
	public static <T> int reflectionHashCode(final int initialNonZeroOddNumber, final int multiplierNonZeroOddNumber, final T object,
		...
		Validator.notNull(object, "The object to build a hash code for must not be null");
	...
	public HashCodeBuilder(final int initialOddNumber, final int multiplierOddNumber) {
		Validator.isTrue(initialOddNumber % 2 != 0, "HashCodeBuilder requires an odd initial value");
		Validator.isTrue(multiplierOddNumber % 2 != 0, "HashCodeBuilder requires an odd multiplier");
