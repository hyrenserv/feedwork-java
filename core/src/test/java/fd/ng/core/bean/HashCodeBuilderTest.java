...
//				123456, 7890, 0), true));
		// cache后的
		assertEquals(((17 * 37 + 0) * 37 + 7890) * 37 + 123456, HashCodeBuilder.reflectionHashCode(new TestSubObject(
				123456, 7890, 0), true));
	}
	...
	public void testReflectionHashCodeExcludeFields() throws Exception {
		...
		// 如果内部获取 Field 使用的是 BeanUtil.getDeclaredFields，对每个 Field 的顺序是：2 3 1
		assertEquals(((17 * 37 + 2) * 37 + 3) * 37 + 1, HashCodeBuilder.reflectionHashCode(x));
		assertEquals(((17 * 37 + 2) * 37 + 3) * 37 + 1, HashCodeBuilder.reflectionHashCode(x, (String[]) null));
		assertEquals(((17 * 37 + 2) * 37 + 3) * 37 + 1, HashCodeBuilder.reflectionHashCode(x));
		assertEquals(((17 * 37 + 2) * 37 + 3) * 37 + 1, HashCodeBuilder.reflectionHashCode(x, "xxx"));
		assertEquals((17 * 37 + 3) * 37 + 1, HashCodeBuilder.reflectionHashCode(x, "two"));
		assertEquals((17 * 37 + 2) * 37 + 1, HashCodeBuilder.reflectionHashCode(x, "three"));

		// 通过JDK原生方法获得Field数组，其顺序和类定义的顺序一致，所以是下面的计算公式
...
//		assertEquals((17 * 37 + 1) * 37 + 2, HashCodeBuilder.reflectionHashCode(x, "three"));
	...
	public void testPerf() {
		...
		TestCaseLog.println("First time : " + (System.currentTimeMillis()-start));

		start = System.currentTimeMillis();
		for(int i=0; i<10_000; i++) {
			int curHash = HashCodeBuilder.reflectionHashCode(x);
		}
		TestCaseLog.println("Often time : " + (System.currentTimeMillis()-start));

		Object[] objs = new Object[]{
				new TestObjectWithMultipleFields(101, 201, 301), new String("sdf"), 234,
		...
		start = System.currentTimeMillis();
		for(int i=0; i<10_000; i++) {
			Object y = objs[i%objs.length];
			int curHash = HashCodeBuilder.reflectionHashCode(y);
		}
		TestCaseLog.println("Often time : " + (System.currentTimeMillis()-start));
	}
