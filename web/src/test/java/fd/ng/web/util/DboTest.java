...
public class DboTest extends FdBaseTestCase {
	@Rule
	public ExpectedException expectedEx = ExpectedException.none();

	public static final String testTableName = "__testdbo_998";
	private static final int Init_Rows = 10; // 向表中初始化的数据条数。必须大于等于10，否则会导致后面的测试用例失败

	@BeforeClass
	public static void start() {
		TestCaseLog.println("Start DboTest......");
	...
	public void after() {
		...
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			Dbo.execute(db,"delete from " + testTableName);
			long nums = Dbo.queryNumber(db, "select count(1) from " + testTableName)
	...
	public static void end() {
		TestCaseLog.println("Over  DboTest......");
		try (DatabaseWrapper db = new DatabaseWrapper()) {
	...
	public void insert() {
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			Result result = Dbo.queryResult(db,
			...
			List<UserForTest> beanList = Dbo.queryList(db, UserForTest.class,
					"select * from " + testTableName + " where age>=? and age<=? order by age",
					-74563, -74561
			);
	...
	public void transaction() {
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			Dbo.rollbackTransaction(db);
			OptionalLong count = Dbo.queryNumber(db,"select count(1) from " + testTableName);
			...
			assertThat(nums, is(1));
			Dbo.commitTransaction(db);
			count = Dbo.queryNumber(db,"select count(1) from " + testTableName);
	...
	public void queryToListMap() {
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			List<Map<String, Object>> result = Dbo.queryList(db,
	...
	public void queryToPagedListMap() {
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			...
			List<Map<String, Object>> result = Dbo.queryPagedList(db, page,
					"select * from " + testTableName + " where age>? and age<? order by age",
					0, 10
			);
			...
			page.setCurrPage(3);
			page.setPageSize(4);
			result = Dbo.queryPagedList(db, page,
					"select * from " + testTableName + " where age>? and age<? order by age",
					0, 10
			);
			assertThat(page.getTotalSize(), is(9));
	...
	public void t41_queryToPagedResult() {
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			Page page = new DefaultPageImpl(1, 3);// 每页3条，查第1页
			Result result = Dbo.queryPagedResult(db, page,
					"select * from " + testTableName + " where age>? and age<? order by age",
					0, 10
			);
			assertThat(page.getTotalSize(), is(9));
			...
			page.setCurrPage(3);
			page.setPageSize(4);
			result = Dbo.queryPagedResult(db, page,
					"select * from " + testTableName + " where age>? and age<? order by age",
					0, 10
			);
			assertThat(page.getTotalSize(), is(9));
	...
	public void queryToArrayList() {
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			List<Object[]> result = Dbo.queryArrayList(db,
					"select * from " + testTableName + " where age>? and age<? order by age desc",
					0, 10
			);
			assertThat(result.size(), is(9));
	...
	public void queryToBeanList() {
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			List<UserBeanForTest> result = Dbo.queryList(db, UserBeanForTest.class,
					"select * from " + testTableName + " where age>? and age<? order by age desc",
					0, 10
			);
			assertThat(result.size(), is(9));
	...
	public void queryToMap() {
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			Map<String, Object> result = Dbo.queryOneObject(db,
	...
	public void queryToBean() {
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			Optional<UserBeanForTest> result = Dbo.queryOneObject(db, UserBeanForTest.class,
					"select * from " + testTableName + " where age=?",
					5
			);
			// 不提取 Opetional 总的对象，直接使用
			assertThat(result.map(UserBeanForTest::getName).get(), is("newUser5"));
			assertThat(result.map(UserBeanForTest::getAge).get(), is(5));
			// 提取对象后做处理
			UserBeanForTest user = result.orElseGet(UserBeanForTest::new); // 没有查询到数据，则会new出来一个新对象。注：orElseGet是延时调用的方式
			assertThat(user.getName(), is("newUser5"));
			assertThat(user.getAge(), is(5));

			// 测试查询不到数据
			Optional<UserBeanForTest> resultNon = Dbo.queryOneObject(db, UserBeanForTest.class,
					"select age,name from " + testTableName + " where age=?",
					123456789
			);
			assertThat(resultNon.map(o -> o.getName()).orElse("no user"), is("no user"));
			UserBeanForTest userNon = resultNon.orElseGet(UserBeanForTest::new);
	...
	public void queryToArray() {
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			Object[] result = Dbo.queryArray(db,
					"select age,name from " + testTableName + " where age=? order by age desc",
					5
			);
	...
	public void queryToNumber() {
		try (DatabaseWrapper db = new DatabaseWrapper()) {
			OptionalLong result = Dbo.queryNumber(db,
